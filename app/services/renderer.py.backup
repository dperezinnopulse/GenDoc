...
class Renderer:
    ...
    def _render_pdf_overlay(self, tpl_path: str, context: Dict[str, Any], original_data: Dict[str, Any], mapping: Dict[str, Any]) -> bytes:
        ...
        def draw_text(key: str, x: float, y: float, value: Any):
            apply_style_for_key(key)\\n            # Get background color from style\\n            style = styles.get(key, default_style)\\n            bg_color = style.get(" background_color\, \#FFFFFF\)\\n # Draw background rectangle if not white\\n if bg_color != \#FFFFFF\:\\n # Convert hex color to RGB\\n bg_r = int(bg_color[1:3], 16) / 255.0\\n bg_g = int(bg_color[3:5], 16) / 255.0\\n bg_b = int(bg_color[5:7], 16) / 255.0\\n # Save current fill color\\n old_fill = c.fillColor\\n c.setFillColor(colors.Color(bg_r, bg_g, bg_b))\\n # Draw background rectangle (estimate text width)\\n text_width = c.stringWidth(str(value) if value is not None else \\, font, size)\\n c.rect(float(x) + offset_x, float(y) + offset_y - 2, text_width + 4, size + 4, fill=1)\\n # Restore original fill color\\n c.setFillColor(old_fill)
